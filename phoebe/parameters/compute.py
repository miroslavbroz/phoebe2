...
def phoebe(**kwargs):
    ...
    * (<phoebe.parameters.ParameterSet>): ParameterSet of all newly created
    ...
    params += [ChoiceParameter(qualifier='etv_method', copy_for = {'kind': ['etv'], 'dataset': '*'}, dataset='_default', value=kwargs.get('etv_method', 'crossing'), choices=['crossing'], description='Method to use for computing ETVs')]
    params += [FloatParameter(visible_if='etv_method:crossing', qualifier='etv_tol', copy_for = {'kind': ['etv'], 'dataset': '*'}, dataset='_default', value=kwargs.get('etv_tol', 1e-4), default_unit=u.d, description='Precision with which to determine eclipse timings')]
